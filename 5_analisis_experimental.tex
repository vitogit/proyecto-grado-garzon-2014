...
El análisis experimental fue realizado en la infraestructura Cluster Fing (sitio web: http://www.fing.edu.uy/cluster), en un procesador AMD Opteron 6272 de 2.09GHz, 48Gb RAM, 64 cores, corriendo el sistema operativo CentOS Linux 6.5.
...
Cuando se realizan comparaciones en los tiempos de ejecución se utiliza el tiempo secuencial que es independiente de la cantidad de procesadores utilizados en las pruebas. 
...
Para las siguientes pruebas la función \emph{fitness} del algoritmo (\ref{eq:funcion_fitness}) tendrá los pesos \emph{x = y = 1}. Estos valores dan pesos equitativos tanto a ómnibus como a otros vehículos por lo que no existe prioridad para uno u otro. Más adelante se realizarán experimentos con otras variantes.
